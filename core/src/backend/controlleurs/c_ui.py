...
def get_goods():
    if user.type_u == 0:
        ...
        query = f"SELECT * FROM BIENS WHERE BIEN.uid_user = {user.uid};"
...
def add_event(date, title, street, city, hours, cp): ...
...
def home_page(pages, table1, table2, QTableWidgetItem):
...
def login(LDAP_CNUSER, LDAP_PASSWORD, pages, username, menu, table1, table2, QTableWidgetItem) -> bool:
